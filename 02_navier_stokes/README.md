...
# Run cavity flow with Boussinesq convection
srun -n 1 -u python nse_cavity.py

# Run rising bubble simulation
srun -n 1 -u python rising_bubble.py
```
...
```python
%cd 02_navier_stokes
!python nse_cavity.py
# or
!python rising_bubble.py
```
...
Both scripts output results to `results/` directory (create if needed: `mkdir -p results/`).

## Visualization

The VTK files can be visualized with **ParaView**:

1. Open ParaView
2. File → Open → select `results/*.pvd` file
3. Click "Apply" in Properties panel
4. Use animation toolbar to step through time
5. Contour or slice plots for detailed flow analysis

## Key Concepts Learned

### Boussinesq Convection (`nse_cavity.py`)
- **Coupled multiphysics**: Momentum and energy equations
- **Buoyancy-driven flow**: Gravity and temperature coupling
- **Singular pressure**: Need for null-space specification
- **Implicit time-stepping**: Stability for coupled problems

### Multiphase Flow (`rising_bubble.py`)
- **Level-set method**: Interface tracking without remeshing
- **Reinitialization**: Maintaining distance function properties
- **Surface tension**: Capillary forces at interface
- **Phase-dependent properties**: Smooth transitions across interface
- **Volume conservation**: Diagnostic measure of solver accuracy
- **Crank-Nicolson time-stepping**: Balance between accuracy and stability

## References

- [Firedrake Project](https://www.firedrakeproject.org/)
- [PETSc](https://petsc.org/)
- Rayleigh-Bernard convection: [doi: 10.1016/j.crme.2008.02.004](https://doi.org/10.1016/j.crme.2008.02.004)
- Rising bubble benchmark: [FeatFlow Database](https://www.mathematik.tu-dortmund.de/~featflow/en/benchmarks/cfdbenchmarking/bubble/bubble_configurations.html)
- Level-set methods: [Wikipedia](https://en.wikipedia.org/wiki/Level-set_method)
- Taylor-Hood elements: Stable finite elements for Stokes flow
